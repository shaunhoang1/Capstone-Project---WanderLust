...
function retrieveStory(){
  ...
  //for(i in combinedText){console.log(combinedText[i])}
  ...
  let groupedImages=[];
  let tempImageNum = 0; //Temp integer at current page to navigate through all story components in this section
  ...
  for(i in combinedText){
    ...
    if(combinedText[tempImageNum].includes("New Section") || combinedText[tempImageNum].includes("FinalPara") ){
        //If next section found, end the search
        sectionFound=true;
        groupedParas.push(currentText);
        currentText=""
    }else if(combinedText[tempImageNum].includes("(IMAGE-") || combinedText[tempImageNum].includes("(VIDEO-")){
      //If an image or video is found, then push them to the image array for loading
      groupedImages[groupedParas.length-1]=combinedText[tempImageNum].slice(12)
    }else if(combinedText[tempImageNum].includes("(OBJECT-")){
        //If an image or video is found, then push them to the image array for loading
        myObjects.push(combinedText[tempImageNum].slice(12))
    }else{  
      //Otherwise add the current text to the section text var
      currentText=currentText+combinedText[tempImageNum]+"\\n";
    }
    tempImageNum++;
  }
  return groupedParas
...
groupedParas=retrieveStory();
console.log(groupedParas)
const fs = require('fs')

// Data which will write in a file.
let data = "\ngroupedParas = [\""+groupedParas.join("\",\"")+"\"];\n";

// Write data in 'Output.txt' .
fs.appendFile('./createWorlds.js', data, (err) => {
